...
while (( "$#" )); do
	...
	echo "$POSITION_CGI_DIR/plot_single_cgi.sh /run/shm/$1 $TYPE -2 $STATION -1 0 0/$PROJ -1"
        "$POSITION_CGI_DIR/plot_single_cgi.sh" /run/shm/Position/$1 $TYPE -2 $STATION -1 0 0 /$PROJ -1
