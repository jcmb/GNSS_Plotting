...
shopt -s nullglob
logger  "do_ren_all.sh started"
echo "do_ren_all.sh started"

for SET in "${RENAME_SETS[@]}"
do
   echo "Ren Processing Set: $SET"
   logger "Ren Processing Set: $SET"
   ...
   do
      ...
      then
         echo "Ren Processing Receiver: $SET:$D"
         logger "Ren Processing Receiver: $SET:$D"
         cd $GNSS_RAW_BASE_DIR/$SET/$D
         find . -not -name $D"_2*.T0[0-9]" -type f| grep .T0 | xargs  -L 1 rdatname.sh $D $1
      fi
   done
done
