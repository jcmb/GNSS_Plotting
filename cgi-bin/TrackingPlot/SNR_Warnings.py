...
def process_arguments():
    ...
    argp.add_argument('--version', action='version', version='%(prog)s v1.1.0')
...
def main():
    ...
    Base_Name = args.Name if args.Name is not None else ""

    if Verbose:
        sys.stderr.write("Base Name: " + Base_Name + "\n")
        sys.stderr.write("Input File: " + str(File.name) + "\n")

    (outside, total, SV_Outside_Tolerance, SV_Measurements) = Process_Single_Band(
        Base_Name, File_Name, File, Tolerance, Number_Outside_Tolerance, Output_File, args.Elev)

    epoch_pct = (float(outside) / float(total) * 100.0) if total else 0.0
    sv_pct = (float(SV_Outside_Tolerance) / float(SV_Measurements) * 100.0) if SV_Measurements else 0.0
    print("{},{},{},{},{},{},{},{}".format(
        Base_Name, File.name, outside, total, epoch_pct,
        SV_Outside_Tolerance, SV_Measurements, sv_pct))
