...
def compute_height_offset(sol_path: str, ats_path: str) -> dict | None:
    ...
    for row in gnss_rows:
        ...
        bracket = _nearest_truth_delta(truth, row["t"])
        if bracket is not None:
            time_deltas.append(bracket)
    ...
    mean_time_delta = sum(time_deltas) / len(time_deltas) if time_deltas else 0.0
    return {
        "matched": len(matched),
        "gnss_mean": gnss_mean,
        "ats_mean": ats_mean,
...
def _nearest_truth_delta(truth, t_query: float) -> float | None:
    if not truth:
        return None
    if t_query <= truth[0].t:
        return truth[0].t - t_query
    if t_query >= truth[-1].t:
        return truth[-1].t - t_query
    for left, right in zip(truth, truth[1:]):
        if left.t <= t_query <= right.t:
            nearest = left.t if abs(left.t - t_query) <= abs(right.t - t_query) else right.t
            return nearest - t_query
    return None
...
def write_report(path: str, stats: dict) -> None:
    lines = [
        ...
        f"Mean GNSS-ATS plot time delta: {stats['mean_time_delta']:.3f} s",
        f"ats_height_offset: {stats['offset']:.4f}",
        f"ats_height_matched: {stats['matched']}",
        f"ats_time_delta_mean: {stats['mean_time_delta']:.3f}",
    ]
